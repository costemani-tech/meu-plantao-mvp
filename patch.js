...
function updateCalendario() {
  ...
  const oldBg = `  const getCellBackground = (ps: PlantaoComLocal[], dia: number) => {
  ...
  const newBg = `  const getCellBackground = (ps: PlantaoComLocal[], dia: number) => {
    if (ps.length === 0) return 'transparent';
    const getCor = (p: PlantaoComLocal) => p.is_extra ? '#8b5cf6' : (p.local?.cor_calendario ?? '#4f8ef7');
...
function updateEscalas() {
  ...
  if (!content.includes("const [tipoJornada, setTipoJornada]")) {
  ...
  const oldPreviewBlock = `  // Preview das 5 primeiras datas em tempo real (permanece no frontend)
  useEffect(() => {
  ...
  const newPreviewBlock = `  // Preview das 5 primeiras datas em tempo real
  useEffect(() => {
    if (dataCompletaISO) {
  ...
  const oldSalvar = `      const parts = regraFinal.split('x');
  ...
  const newSalvar = `      let regraGravar = regraFinal;
      let trabalhoA = 12;
      let descansoA = 36;
  ...
  content = content.replace(oldSalvar, newSalvar);
  ...
  const oldRegraGui = `<div className="form-group">
  ...
  content = content.replace(oldRegraGui, newRegraGui);

  const fechamentoPlantonista = `</div>
            )}
          </div>

          <div className="form-group" style={{ `;
          
  const novoFechamento = `</div>
            )}
          </div>
          ) : (
          <div className="form-group">
  ...
  content = content.replace(fechamentoPlantonista, novoFechamento);
...
function updateScaleGenerator() {
  ...
  const oldFun = `export function gerarProximosPlantoes(
  dataInicio: Date,
  regra: Regra,
  quantidade: number = 5
): SlotPlantao[] {
  ...
  const newFun = `export function gerarProximosPlantoes(
  dataInicio: Date,
  regra: Regra,
  ...
  content = content.replace(oldFun, newFun);
  fs.writeFileSync(path, content, 'utf8');
}
