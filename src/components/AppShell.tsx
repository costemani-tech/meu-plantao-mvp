...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  // Redirecionamento de sessão existente no client-side
  useEffect(() => {
  ...
  // Skip render in special routes (including our new processing page)
  if (pathname === '/login' || pathname === '/' || pathname === '/auth/processing') {
    return <>{children}</>;
  }
    ...
    <div className="app-shell">
      ...
      {loading && (
        <div style={{
          ...
          <div className="hidden md:grid hero">
            <div className="hero-content">
              <div className="w-16 h-16 bg-blue-600/10 rounded-2xl flex items-center justify-center text-blue-500 border border-blue-500/20 shadow-[0_0_30px_rgba(59,130,246,0.2)] animate-pulse mb-6">
              ...
              <p className="hero-subtitle">
                Organize seus plantões sem complicação.
              </p>
              {/* Premium Progress Bar */}
              <div className="w-full max-w-[320px] h-1.5 bg-slate-800 rounded-full overflow-hidden mt-8 relative">
                <div className="h-full bg-[#3B82F6] rounded-full animate-pulse" style={{ width: '60%' }}></div>
              </div>
            </div>
