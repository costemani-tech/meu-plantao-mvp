...
export default function DashboardInteractive({ initialIsPro, userId }: DashboardInteractiveProps) {
  const router = useRouter();
  const [plantoes, setPlantoes] = useState<PlantaoComLocal[]>([]);
  const [loading, setLoading] = useState(true);
  const [gerandoPdf, setGerandoPdf] = useState(false);
  const [showProModal, setShowProModal] = useState(!initialIsPro);
  ...
  useEffect(() => {
    ...
    fetchPlantoes();
  }, [fetchPlantoes, initialIsPro]);

  const exportPDF = async () => {
    if (!reportRef.current || !initialIsPro) return;
    setGerandoPdf(true);
    try {
