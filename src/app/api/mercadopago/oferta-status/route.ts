...
export async function GET() {
  try {
    ...
    if (error) {
      ...
      return NextResponse.json({ ofertaAtiva: false, proCount: 0, error: 'Erro ao verificar o status da oferta' }, { status: 500 });
    }
    ...
    return NextResponse.json({ ofertaAtiva: false, proCount: 0, error: 'Erro interno ao verificar o status da oferta' }, { status: 500 });
  }
}
