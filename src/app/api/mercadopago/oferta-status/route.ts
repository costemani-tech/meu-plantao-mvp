...
export async function GET() {
  try {
    ...
    if (error) {
      ...
      return NextResponse.json({ ofertaAtiva: false, proCount: 0, error: 'Erro interno ao consultar status da oferta' }, { status: 500 });
    }
    ...
    return NextResponse.json({ ofertaAtiva: false, proCount: 0, error: 'Erro interno ao processar status da oferta' }, { status: 500 });
  }
}
