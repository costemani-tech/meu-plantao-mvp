  ...
  return (
    <>
      <div className="page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          <h1>Calendário 📅</h1>
          <p>Visualize seus plantões no mês</p>
        </div>
      ...
      <div className="card">
        ...
        <div className="calendar-grid">
          {cells.map((cell, idx) => {
            ...
            return (
              <div key={idx} className={`cal-day ${cell.mesAtual ? '' : 'other-month'} ${cell.mesAtual && isHoje(cell.dia) ? 'today' : ''}`}>
      ...
      <div style={{ marginTop: 24 }}>
        <h2 style={{ fontWeight: 700, marginBottom: 16, fontSize: 16 }}>Plantões de {MESES[mes]}</h2>
        {plantoes.length === 0 ? (
          <div className="card">
            <div className="empty-state">
              <div className="empty-icon">📅</div>
              <p>Nenhum plantão neste mês.</p>
            </div>
          </div>
        ) : (
